...
- Serialization is an in-memory snapshot: `world.serialize()` returns a columnar object (`version: 2`, omits `skipSerialize`); `new World(snapshot)` restores it (also accepts legacy `version: 1` entity lists). For persistence, custom encode/decode is needed. Use `world.dump()` for debug exports that include `skipSerialize` components (not for restore).
